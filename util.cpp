...
int encodeNetascii(const char *fileName) {
    FILE *fp = nullptr, *tfp = nullptr;
    char tempFileName[1024];
    strcpy_s(tempFileName, 1023, fileName);
    strcat_s(tempFileName, 1023, ".netascii");
    fopen_s(&fp, fileName, "rb");
    fopen_s(&tfp, tempFileName, "wb");
    if (fp == nullptr || tfp == nullptr) {
        if (fp) { fclose(fp); }
        if (fp) { fclose(tfp); }
        return -1;
    }

    // 读取文件并进行编码,主要干的事情就是将LF转换为CRLF,CR转换为CRNUL
    int ch = fgetc(fp), prev_ch = NUL;
    while (true) {
        if (ch == LF && prev_ch != CR) { fputc(CR, tfp); }  // LF => CRLF
        else if (prev_ch == CR && ch != LF && ch != NUL) { fputc(NUL, tfp); }   // CR => CRNUL
